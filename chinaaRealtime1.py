...
def TFST(df):  # 25-60
    ...
    if df.loc[len(df) - 1, 'ma25'] > df.loc[len(df) - 2, 'ma25'] and df.loc[len(df) - 2, 'ma25'] > df.loc[
        ...
        if df.loc[len(df) - 1, 'ma25'] - df.loc[len(df) - 1, 'ma5'] > 0 and df.loc[len(df)-1,'ma25']-df.loc[len(df)-1,'ma5']<0.2 and df.loc[len(df)-1,'ma5']>df.loc[len(df)-2,'ma5'] and df.loc[len(df)-2,'ma5']>df.loc[len(df)-3,'ma5']:
...
def plus2560(df): #激进版25-60,KDJ金叉当天，MACD低点，MA5在MA25下且最后一天拐头，MA25多头，RSI-14低点拐头
    plus2560value=' '
...
liststockcode=pd.read_csv('chinaacode0715-1.csv')
...
while time.strftime('%H:%M', time.localtime()) > '09:30' and time.strftime('%H:%M',
    ...
    for tablename in liststockcode['symbol']:
        ...
        # 激进版25-60,KDJ金叉后一天，MACD低点，MA5在MA25下且最后一天拐头，MA25多头
        if plus2560value!=' ':
